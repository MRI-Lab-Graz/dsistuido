...
def inspect_fib_file(fib_path: Path) -> int:
    """Inspect one FIB file and print all non-internal keys and shapes."""
    if not fib_path.exists():
        print(f"Error: File not found: {fib_path}")
        return 1

    print(f"Inspecting {fib_path}")
    try:
...
def main():
    ...
    parser.add_argument('fib_dir', nargs='?', help='Directory containing FIB files')
    ...
    parser.add_argument('--inspect', help='Inspect one FIB file and print keys/shapes')
    parser.add_argument('--method', choices=['gqi', 'qsdr', 'all'], default='all', 
                       help='Filter by reconstruction method')
    args = parser.parse_args()

    if args.inspect:
        sys.exit(inspect_fib_file(Path(args.inspect)))

    if not args.fib_dir:
        parser.error('fib_dir is required unless --inspect is provided')
